...
/*
  arrListStud - array of students
  studLL - LL of students
  studDict - open dictionary: categorized by year level
  dcismDict - closed dictionary: categorized by programs
  studRec - another form of the student structure
  studSet - array of studRecs
*/
...
arrListStud populateStudentList(void)
{
    ...
    if((fp = fopen("students.bin", "rb"))== NULL){
        ...
        for(;SL.numStuds<dataCount;SL.numStuds++){
          SL.studs[SL.numStuds] = studs[SL.numStuds];
        }
    }
    return SL;
}
