...
/* 
after completing everything, we can add more functionalities like grades and passers list struct
insert functions
delete functions
update grades functions
search student with a specific id num
*/
...
void sortAlphabetically(ClassListPtr C){
    ...
    for(i=0;i<C->ctr;i++){
        min = i;
        for(j=i+1;j<C->ctr;j++){
        	if(strcmp(C->class[min].stud_name.fname, C->class[j].stud_name.fname) < 0){
        		min = j;
			}
		}
		temp = C->class[min];
        ...
        while(back){
            switch(getChoice(classChoice)){
                ...
                case 5:
                    ...
                    break;
                case 6:
                    sortAlphabetically(chosenClass);
                    break;
...
void displayStudentsByYear(ClassListPtr C) {
    ...
    for (i = 0; i < C->ctr; i++) {
        ...
        }// you forgot to account for no students found born on that year
    }
}
